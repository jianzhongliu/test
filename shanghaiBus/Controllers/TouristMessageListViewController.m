...
@interface TouristMessageListViewController () <UITableViewDataSource, UITableViewDelegate>

@property (nonatomic, strong) UITableView *tableView;

@end

@implementation TouristMessageListViewController
...
- (void)initUI {
    ...
    NSString *stringComment = [NSString stringWithFormat:@"%ld条留言", (long)self.arrayMessage.count];
    ...
    [buttonAddComment setTitle:@"写留言" forState:UIControlStateNormal];
    [buttonAddComment addTarget:self action:@selector(didClickAddComment) forControlEvents:UIControlEventTouchUpInside];
    
    buttonAddComment.titleLabel.font = [UIFont systemFontOfSize:15];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return [cell fetchCellHightWithData:[self.arrayMessage objectAtIndex:indexPath.row]];
}
